  ...
  `;
   ...
   `;
	...
	it("nullifies a creditaccount, setting available values to 0 and isActive to false", async () => {
		...
		const response = await request(app).post("/graphql").send({query: createAccountToNullifyMutation})
		expect(response.status).toBe(200)
		...
		`
		const nullificationResponse = await request(app).post("/graphql").send({query: nullifyAccountMutation})
		...
		expect(nullificationResponse.body.data.nullifyAccount.availableCredits).toBe(0)
		expect(nullificationResponse.body.data.nullifyAccount.availableMoney).toBe(0)
		expect(nullificationResponse.body.data.nullifyAccount.originalCredits).toBe(initialValue)
		expect(nullificationResponse.body.data.nullifyAccount.originalMoney).toBe(initialValue)
