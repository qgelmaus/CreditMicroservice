		...
		try {
			const res = await createGiftAccount({
				variables: {
					input: {
						email: giftEmail,
						purchaseAmount: Number.parseFloat(giftAmount),
					},
				},
			});
			...
			const res = await createPrepaidAccount({
				variables: {
					input: {
						email: prepaidEmail,
						treatmentCount,
						pricePerTreatment: Number.parseFloat(pricePerTreatment),
					},
				},
			});
					...
					<div style={{ marginTop: "1rem" }}>
						...
						<label>
							...
							<select
								...
								onChange={(e) =>
									setTreatmentCount(Number.parseInt(e.target.value))
								}
								style={{ padding: "0.5rem", width: "100%" }}
							>
						...
						{pricePerTreatment && (
							<div style={{ marginTop: "1rem" }}>
								{(() => {
									const raw =
										treatmentCount * Number.parseFloat(pricePerTreatment);
